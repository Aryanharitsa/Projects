...
async def aml_network_rules() -> Dict[str, Any]:
    async with httpx.AsyncClient(timeout=10) as client:
        r = await client.get(f"{AML_SVC}/aml/network/rules")
        if r.status_code >= 400:
            raise HTTPException(status_code=r.status_code, detail=r.json().get("detail", r.text))
        return r.json()
...
async def aml_network_analyze(payload: Dict[str, Any]) -> Dict[str, Any]:
    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.post(f"{AML_SVC}/aml/network/analyze", json=payload)
        if r.status_code >= 400:
            raise HTTPException(status_code=r.status_code, detail=r.json().get("detail", r.text))
        return r.json()
...
async def aml_network_counterfactual(payload: Dict[str, Any]) -> Dict[str, Any]:
    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.post(f"{AML_SVC}/aml/network/counterfactual", json=payload)
        if r.status_code >= 400:
            raise HTTPException(status_code=r.status_code, detail=r.json().get("detail", r.text))
        return r.json()
...
async def aml_network_attribution(payload: Dict[str, Any]) -> Dict[str, Any]:
    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.post(f"{AML_SVC}/aml/network/attribution", json=payload)
        if r.status_code >= 400:
            raise HTTPException(status_code=r.status_code, detail=r.json().get("detail", r.text))
        return r.json()
...
# ----- Triage pass-through (round-16, day-75)


@app.get("/aml/triage/rules")
async def aml_triage_rules() -> Dict[str, Any]:
    async with httpx.AsyncClient(timeout=10) as client:
        r = await client.get(f"{AML_SVC}/aml/triage/rules")
        r.raise_for_status()
        return r.json()


@app.get("/aml/triage/profile")
async def aml_triage_profile() -> Dict[str, Any]:
    async with httpx.AsyncClient(timeout=25) as client:
        r = await client.get(f"{AML_SVC}/aml/triage/profile")
        if r.status_code >= 400:
            raise HTTPException(status_code=r.status_code, detail=r.text)
        return r.json()
...
async def aml_triage_candidates(
    limit: int = 100,
    include_closed: bool = False,
) -> Dict[str, Any]:
    async with httpx.AsyncClient(timeout=15) as client:
        r = await client.get(
            f"{AML_SVC}/aml/triage/candidates",
            params={"limit": limit, "include_closed": include_closed},
        )
        if r.status_code >= 400:
            raise HTTPException(status_code=r.status_code, detail=r.text)
...
async def aml_triage_case(case_id: str) -> Dict[str, Any]:
    async with httpx.AsyncClient(timeout=25) as client:
        r = await client.get(f"{AML_SVC}/aml/triage/case/{case_id}")
        if r.status_code >= 400:
            try:
                detail = r.json().get("detail", r.text)
            except Exception:
...
async def aml_triage_seed(force: bool = False) -> Dict[str, Any]:
    async with httpx.AsyncClient(timeout=60) as client:
        r = await client.post(
            f"{AML_SVC}/aml/triage/seed",
            params={"force": force},
        )
        if r.status_code >= 400:
            raise HTTPException(status_code=r.status_code, detail=r.text)
...
@app.get("/aml/triage/export.md")
async def aml_triage_export(case_id: str):
    from fastapi.responses import PlainTextResponse
    async with httpx.AsyncClient(timeout=25) as client:
        r = await client.get(
            f"{AML_SVC}/aml/triage/export.md",
